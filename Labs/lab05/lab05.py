...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[int(ii/2)] if ii%2==0 else deck[int(len(deck)/2+ii/2)] for ii,i in enumerate(deck)]
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) + is_leaf(t2) >=1:
        ...
        branches_merged = [add_trees(i[0],i[1]) for i in zip(b1,b2)]
        if len(b1) < len(b2):
            # branches_unique = b2[len(b1):]
            branches_merged = branches_merged+b2[len(b1):]  # take only one var branches_merged for recursion
        elif len(b2) < len(b1):
            # branches_unique = b1[len(b2):]
            branches_merged = branches_merged+b1[len(b2):]
        return tree(label=label(t1)+label(t2),
                    branches= branches_merged)
