...
def balanced(m):
    ...
    True
    """
    # Why this is not correct?????? =_=????
    # l_arm = left(m)
    # r_arm = right(m)
    # b = True
    # if is_planet(end(l_arm)) or is_planet(end(r_arm)):
    #     total_weight_l = total_weight(end(l_arm))
    #     total_weight_r = total_weight(end(r_arm))
    #     b = length(l_arm)*total_weight_l == length(r_arm)*total_weight_r
    #     return b
    # else:
    #     return b and balanced(end(l_arm)) and balanced(end(r_arm))
...
def quadratic(x, a, b, c):
    ...
    if t1<=q<=t2:
        return interval(min(f(q),f(t1),f(t2)),max(f(q),f(t1),f(t2)))
    else:
        return interval(min(f(t1),f(t2)),max(f(t1),f(t2)))
