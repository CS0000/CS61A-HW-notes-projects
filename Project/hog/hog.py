...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    # If you call it fewer or more than that, 
    # it won't be at the right spot in the cycle for the next roll
...
# python3 ok -q 05 -u   the last question: 
# for player0:
# score=71, opp = 80
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    """
    # BEGIN PROBLEM 5
    # just print everything out then it would be clear
    who=0
    while score0<goal and score1<goal:
        # print(f"who: {who}")
        # print(f"whether scores are smaller than goal: {score0<goal and score1<goal}")
        if who == 0:
            add_score = take_turn(strategy0(score0, score1), score1, dice)
            score0 += add_score
            # print(f"add {add_score} to score0")
            say = say(score0,score1)
            # print(f"score0: {score0}, score1: {score1}")
            if score0<goal:   # score changes after you add the current score on it
                if extra_turn(score0,score1)==False:  # if there is no extra turn, change player
                    
                    who = other(who)
                    # print(f"change player to {who}")
                else:   # if there is an extra turn, move back to "while score0<goal and score1<goal"
                    # print('there is an extra_turn, continue')
                    continue

        if who ==1:
            add_score = take_turn(strategy1(score1, score0), score0, dice)
            score1 += add_score
            # print(f"add {add_score} to score1")
            say = say(score0,score1)
            # print(f"score0: {score0}, score1: {score1}")
            if score1<goal:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who ==0:
            # print(f"who: {who}")
            # print(f'last_score is {last}, score_0 is {score0}, high is {high}')
            if score0-last > high:
                print(f"{score0-last} point(s)! The most yet for Player {who}")
                high = score0-last
                # print(f"reassign high to {high}")
            else:
                pass
            last = score0
        ...
        if who == 1:
            # print(f"who: {who}")
            # print(f'last_score is {last}, score_1 is {score1}, high is {high}')
            if score1-last > high:
                print(f"{score1-last} point(s)! The most yet for Player {who}")
                high = score1-last
                # print(f"reassign high to {high}")
            else:
                pass
...
def make_averaged(original_function, trials_count=1000):
    ...
    def f(*args):
        ...
        for i in range(trials_count):
            values += original_function(*args)
        return values / trials_count
    return f
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    averaged_roll_dice_f = make_averaged(roll_dice, trials_count)
    max_ = averaged_roll_dice_f(10,dice)
    max_ind = 10  # start from the largest num_roll, so that it will return the lowest num_roll
    i = 10
    while i > 0:
        v = averaged_roll_dice_f(i,dice)
        if v >= max_:
            max_ = v
            max_ind = i
        i -= 1
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    In other cases, 
    it rolls 0 if rolling 0 would give at least cutoff points. Otherwise, the strategy rolls num_rolls.
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score, opponent_score):
        ...
        num = bacon_strategy(score, opponent_score, cutoff=cutoff, num_rolls=num_rolls)
        return num
    # END PROBLEM 11
